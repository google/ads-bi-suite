...
AS (
  SELECT UPPER(device_id_md5) AS device_id, affinity_name,
  FROM adh.google_ads_impressions_rdid, UNNEST(affinity) AS affinity_id
  LEFT JOIN adh.affinity
    USING (affinity_id)
