...
CREATE TEMP FUNCTION getKeywordMatchType(status INT64)
  AS (['UNSPECIFIED','UNKNOWN','EXACT','PHRASE','BROAD'][OFFSET(status)]);

select distinct
  a.customer.descriptiveName as Account,
  a.customer.id as Customer_ID,
  a.adGroup.name as Ad_group_name,
  a.adGroup.id as Ad_group_id,
  ...
  a.adGroupCriterion.keyword.text as Keyword,
  getKeywordMatchType(a.adGroupCriterion.keyword.matchType) as Keyword_match_type,
